...
execute store result storage call_stack: this.matches_size int 1 run data get storage call_stack: this.matches
...
execute if data storage call_stack: {this:{matches_size:1}} unless data storage call_stack: this.matches[0].slots run data modify storage call_stack: this.matching_slots append value -1
execute if data storage call_stack: {this:{matches_size:1}} if data storage call_stack: this.matches[0].slots run data modify storage call_stack: call.arg0 set from storage call_stack: this.matches[0].slots
execute if data storage call_stack: {this:{matches_size:1}} if data storage call_stack: this.matches[0].slots run data modify storage call_stack: call.arg1 set from storage call_stack: this.slots
execute if data storage call_stack: {this:{matches_size:1}} if data storage call_stack: this.matches[0].slots run function dt.crafting_util:internal/array/split_by_key
execute if data storage call_stack: {this:{matches_size:1}} if data storage call_stack: this.matches[0].slots run data modify storage call_stack: this.matching_slots set from storage call_stack: call.result[0]

#tellraw @p [{"text":" matching slots: "},{"nbt":"this.matching_slots","storage":"call_stack:"}]
#tellraw @p [{"text":" ingr: "},{"nbt":"this.ingredients","storage":"call_stack:"}]
#tellraw @p [{"text":" match: "},{"nbt":"this.matches[0].slots","storage":"call_stack:"}]

# If the item_count did not match, add it to the remaining as it could be a tag
# TODO add has tags flag, then fail early instead of just adding to remaining
execute unless data storage call_stack: this.matching_slots[0] run data modify storage call_stack: this.remaining append from storage call_stack: this.item_counts[0]
...
#tellraw @p [{"text":"ingredients: "},{"nbt":"this.ingredients[0]","storage":"call_stack:"}]
