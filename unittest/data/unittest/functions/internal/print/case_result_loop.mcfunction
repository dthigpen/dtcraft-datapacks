...
execute if data storage unittest:internal {temp:{return:{pass:true}}} run tellraw @p ["    ",{"text":"Pass: ","color":"dark_green"},{"nbt":"temp.result.msg","storage":"unittest:internal", "interpret": true}]
execute unless data storage unittest:internal {temp:{return:{pass:true}}} run tellraw @p ["    ",{"text":"Fail: ","color":"dark_red"},{"nbt":"temp.result.msg","storage":"unittest:internal", "interpret": true}]
