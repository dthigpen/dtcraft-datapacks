...
execute if data storage unittest:internal {temp:{result:{pass:true}}} run tellraw @s ["    ",{"text":"Pass!: ","color":"dark_green"},{"nbt":"temp.result.msg","storage":"unittest:internal", "interpret": true}]
execute unless data storage unittest:internal {temp:{result:{pass:true}}} run tellraw @s ["    ",{"text":"FailA: ","color":"dark_red"},{"nbt":"temp.result.msg","storage":"unittest:internal", "interpret": true}]
