...
function dt.autocraft:internal/blocks/autocrafter/placeholders/add_tag_to_item
data modify storage call_stack: this.tagged append from storage call_stack: call.result
data remove storage call_stack: this.items[0]

execute unless data storage call_stack: this.items[0] run data modify storage call_stack: this.result set from storage call_stack: this.tagged
