...
execute unless data entity @s SelectedItem if data storage dtcraft:call_stack this.this_user.data.selected run data modify storage dtcraft:call_stack this.this_user set from storage dtcraft:call_stack this.calls[0].value
# tellraw @p ["me: ",{"nbt":"this.this_user","storage":"dtcraft:call_stack"}]
#region update user
data modify storage dtcraft:call_stack call.arg0 set from storage dtcraft:refill users
