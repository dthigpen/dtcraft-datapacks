        ...
        with directory("internal"):
            ...
            def do_refill():
                with scope():
                    ...
                    with execute(
                        unless(CurrentEntity().to_path("SelectedItem").present())
                    ):
                        call(remove_item_fn, slot_from_inv)

                    call(replace_item_fn, item)
                    call(print_did_refill, slot_from_inv)
            ...
            def refill_from_inventory():
                with scope():
                    ...
                    tmp_ret = call(
                        by_key_limit_fn, user.key("inventory"), search_item, 1
                    )
                    ...
                    with execute(if_(stored_item_found.key("id").present())):
                        ...
                        with execute(if_(actual_item_found.key("id").present())):
                            call(do_refill, user, actual_item_found)
            ...
            def try_refill():
                with scope():
                    ...
                    with execute(
                        if_(user_slot.equals(player_slot), user_count.matches(1))
                    ):
                        call(refill_from_inventory, user)
            ...
            def check_refills():
                with scope():
                    ...
                    with execute(
                        ...
                        tmp_user = call(try_refill, user)
                        user.set(tmp_user)

                    with execute(if_(entity_selected.present())):
                        user.set(call(set_user_item_data, user))
