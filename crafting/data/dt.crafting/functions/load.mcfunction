...
scoreboard players set $dt.crafting.version.major load.status 0
scoreboard players set $dt.crafting.version.minor load.status 1
...
execute unless score $dt.array load.status matches 1 run tellraw @p ["",{"text":"Error: ","color":"dark_red"},{"text":"Crafting","bold":true}," requires ",{"text":"Array ","bold":true},"0.1"]
scoreboard players set $dt.tmp.dep load.status 0
